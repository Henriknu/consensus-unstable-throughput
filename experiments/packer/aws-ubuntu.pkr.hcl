...
source "amazon-ebs" "ubuntu" {
  ...
  ami_name      = "abft-ubuntu-ami-${local.timestamp}"
...
build {
  ...
  inline = [
    ...
    "sudo apt-get install -y iproute2 build-essential make automake autoconf libtool pkg-config openssl",
  ]
}
  ...
  inline = [
    ...
    ". $HOME/.cargo/env",
    "cargo build --release",
    "cp target/release/abft $HOME/app/abft",
  ]
}
